...
OUTCOMES = Path(
    os.environ.get(
        "IJGGC_OUTCOMES_PATH",
        ROOT / "evidence" / "IJGGC_V2_VALIDATION_OUTCOME_PROFILES.csv",
    )
)
...
REPORT = ROOT / "validation" / f"WP8_IJGGC_FIGURE_TABLE_VALIDATION_{VERSION}.md"
...
def main() -> int:
    ...
    table02 = read_csv(TABLE_ROOT / "Table02_core_evidence_summary.csv")
    outcome_profiles = read_csv(OUTCOMES)
    expected_outcomes = {
        row["outcome"]: as_int(row["positive_n"])
        for row in outcome_profiles
        if row.get("application_group") == "ALL"
    }
    table02_columns = {
        "direct_field_validation": "field_evaluation",
        "controlled_release_ml_validation": "controlled_event",
        "explicit_ood_testing": "OOD",
        "explicit_cross_site_testing": "cross_site_or_setting",
        "uncertainty_calibration_or_posterior_check": "calibrated_UQ_or_posterior",
        "any_physical_diagnostic_checked": "physical_diagnostic",
        "surrogate_error_propagation": "surrogate_error_propagation",
    }
    for outcome, column in table02_columns.items():
        actual = sum(as_int(row[column]) for row in table02)
        expected = expected_outcomes.get(outcome, -1)
        check(
            f"Table 2 total matches final outcome profile for {outcome}",
            actual == expected,
            f"table={actual}; profile={expected}",
        )
    ...
    check("at least six supplementary tables exist", len(supplement) >= 6, f"count={len(supplement)}")
    forbidden_patterns = [
        re.compile(r"[A-Za-z]:\\"),
